...
def _bloat_canonical_cv(package, *, count=25, host_managed_optional=True):
    ...
    for index in range(count):
        blocks.append({
            "id": f"overflow-{index}",
            "type": "bullet",
            # Twenty-five rendered bullets on top of the master: enough real
            # paragraphs to push the document onto a second page.
...
def test_an_empty_paragraph_costs_a_real_line(tmp_path):
    """Empty paragraphs are not free: LibreOffice gives them a full line.

    The page budget is a measurement now, so this is asserted by rendering
    rather than by modelling.  A document of blank paragraphs that still fits
    one page is fine; one that overflows must be caught.
    """

    from tools.workflow.materials_renderer import _measure_document_pages

    document = Document()
    for _ in range(45):
        document.add_paragraph()
    pages = _measure_document_pages(document, material="cv")
...
def test_render_blocks_over_budget_before_writing_anything(tmp_path):
    ...
    # A previous generation may carry a frozen artifact receipt.  A rejected
    # render must not discard it before the new page count is known.
    (package / "artifact_hashes.json").write_text('{"previous_generation":"intact"}', encoding="utf-8")
    before_hashes = (package / "artifact_hashes.json").read_bytes()
    old_cv = package / "Old CV.pdf"
    old_cv.write_bytes(b"prior generation; must not be archived by a failed render")
    ...
    try:
        ...
        raise AssertionError("expected a capacity block for a 25-bullet overflow")
    assert message.startswith("capacity_budget_exceeded:cv:")
    assert message.endswith("pages")
    ...
    assert (package / "artifact_hashes.json").read_bytes() == before_hashes
    assert sorted(path.name for path in package.glob("*.pdf")) == before_pdfs
    assert old_cv.read_bytes() == b"prior generation; must not be archived by a failed render"
...
def test_render_stage_reports_page_budget_with_revision_scope(tmp_path):
    ...
    out = dispatch("materials", workspace=ws, payload={"job_id": "C0-001", "stage": "render"})
    assert out["status"] == "blocked"
    assert out["blockers"] == ["capacity_budget_exceeded"]
    assert out["next_action"] == "revise_only_over_budget_materials"
    assert out["capacity_gate"]["materials"] == ["cv"]
    assert "capacity_budget_exceeded:cv:" in out["error"]
