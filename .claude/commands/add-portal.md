...
You are helping the user build a job-portal search skill for a job board in their market. The repo ships four current portal examples (LinkedIn, JobsDB, CTgoodjobs and FreeHire). This command investigates another portal, scaffolds a skill from the canonical structure, and test-runs a live query before registering anything.
...
3. **Market and language** - which country/region the portal covers and what language its postings use. This drives the trigger phrases in `SKILL.md`; include local-language terms where useful.
...
These conventions are what make portal skills interchangeable for `/scan` and for users reading any skill's docs:
...
- **`tests/`:** add `runCLI`/`parseJSON` helpers, then add a small live smoke-test file: `search` with the test query returns exit code 0 and ≥1 result with non-null `id`/`title`/`url`; a bogus flag or missing required arg exits 1 with a JSON error on stderr.
