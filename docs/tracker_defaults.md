...
- **入表时才分配**：扫描预览不产生岗位编号；确认入表时，`tools/workflow/id_allocation.py` 从工作区本地的 `02_Tracker/workflow/id_counters.json` 读取每个 lane 字母的最新编号，必要时以本地台账和已存在材料包引导初始化，保留同 URL 的已有正式编号，再由 `tools/fresh_24h/job_id.py` 的 `allocate_ids` 分配新编号。核心/一级/二级只决定编号第二位和文件夹路由，不产生各自的计数器；同一 lane 共享一个连续序列，且序号始终为三位数。确认后才推进计数器，Google Sheets 不再作为编号计数器。
...
- **材料状态格式是固定协议**：当前表头中的 `材料状态` 固定为 V 列。每日第一次创建 `fresh_24h_YYYY-MM-DD` 工作表时，代码写入从第 2 行开始的下拉菜单（`未制作`、`已制作`、`已投递`、`面试中`、`已结束`、`已录用`），并写入 `已投递` 触发整行绿色底色的条件格式。材料内容和格式门全部通过后，系统自动把绑定行改为 `已制作`；后续追加和受保护迁移只继承/重申这一格式，模型不能自行决定下拉选项、列位置、颜色或完成状态。
