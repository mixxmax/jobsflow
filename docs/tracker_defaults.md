...
- **材料状态格式是固定协议**：当前表头中的 `材料状态` 固定为 V 列。每日第一次创建 `fresh_24h_YYYY-MM-DD` 工作表时，代码写入从第 2 行开始的下拉菜单（`未做`、`已定制`、`已投递`、`面试中`、`已结束`、`已录用`），并写入 `已投递` 触发整行绿色底色的条件格式。后续追加和受保护迁移只继承/重申这一格式，模型不能自行决定下拉选项、列位置或颜色。
