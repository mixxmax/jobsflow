...
| `/intake` | Accept URLs you already reviewed; normalize, deduplicate and fill page metadata before a read-only proposal | Provide URLs and, when needed, title, employer, lane or full JD | A proposal containing only new jobs; missing full JD is marked `待审-JD不足` with no fabricated score | Only explicit confirmation allocates IDs and writes the tracker |
