...
- **lane 與編號邊界重新固定**：深評邊界只鎖定 URL 對應的 lane 字母；掃描預覽不分配永久編號。只有用戶確認入表時，系統才按每個 lane 字母的唯一最新計數器順延三位數編號；核心/一級/二級只決定第二位和包路由，不產生獨立計數器，並將編號、台帳行和材料包綁定。
- **材料狀態格式固定**：fresh24 工作表首次建立時，V 列「材料狀態」由代碼寫入固定下拉選項；材料內容和格式門通過後，系統自動寫入「已製作」；選擇「已投遞」會以條件格式把整行變成綠色。後續追加只沿用這套格式，不由模型決定列位、選項、顏色或完成狀態。
