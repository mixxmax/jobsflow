...
- **CV/CL 先審內容，再製作文件**：主模型先保存結構化 canonical CV/CL；系統隨後自動派發一個獨立上下文的子 Agent，只讀緩存 JD、完整 CV/CL 和精簡審計規則，集中核查 JD 回應、STAR、LLMO 證據位置、跨材料一致性、殘句及模板殘留。子 Agent 不審 Email、DOCX/PDF 或版式，也不直接改材料；同一模型可在獨立上下文中承擔審計，流程不綁定特定模型供應商。
...
- 主模型先保存帶穩定 block ID 的 canonical CV/CL；系統自動啟動精簡的獨立內容審計，子 Agent 只讀 JD、完整 CV/CL、內部覆蓋處置和編譯規則，不讀長手冊、事實庫、Email、DOCX/PDF 或版式；沒有真實正面證據的要求只在內部標記為 `intentionally_omitted`，不得改寫成主動暴露缺點的材料文字；P0/P1 只允許主模型修改 finding 指向的 block，內容通過後才統一生成 DOCX/PDF；每崗最多三審、同一問題第二次仍出現即熔斷；
