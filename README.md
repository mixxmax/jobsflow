...
- **從「模型自覺遵守」升級為代碼約束的工作流**：統一網關、政策規則、狀態機、任務包和事後門禁接通 `/scan → /push → /materials → /apply`；掃描只展示 lane/評分，不入表、不分配永久編號；`/push` 必須預覽後經用戶確認，歸檔必須預覽確認，`/apply` 永不自動提交。
- **召回、評分和成本分開控制**：URL 級 JD 緩存優先複用，摘要缺失/過短和灰區崗位會被救援；深取受節能/平衡/廣覆蓋預算控制，完整 JD 決定深評，未取到全文的崗位保留為 `待審-JD不足`，不會靜默丟失。
- **基礎版是內容母版，CV/CL 先審內容再製作文件**：CV 與 CL 分別從各自 lane 基礎版增量定制，共用同一用戶畫像與能力上沿，但互不充當事實來源，也不讀其他崗位材料。主模型只提交當前崗位綁定的 JD 定制增量；系統保留未修改內容，拒絕整份替換或無聲縮減，再自動派發獨立上下文核對 JD 回應、STAR、LLMO 位置、職位/雇主邊界、語法與殘句。子 Agent 不審 Email、DOCX/PDF 或版式；Email 在內容通過後由宿主固定生成。
- **返修有邊界，輸出不走捷徑**：子 Agent 只返回帶段落定位的 findings，主模型只修改命中的內容塊，再交回獨立審計；每崗最多三審，同一問題重複出現會熔斷並轉人工檢視。內容通過後，系統才從對應 lane 的基礎 DOCX 統一渲染並轉 PDF，頁數、文字層、文件名和元數據由確定性檢查負責。
- **本地優先的臺帳同步**：本地 ledger 是崗位事實源，本地 CSV 是可直接使用的表格；Google Sheets 只是可選投影。同步失敗可重放，遠端變化先 reconcile，用戶字段只能通過明確的 `sync pull` 導入。
- **入表展示有固定協議**：用戶確認入表後，新批次固定插在表頭下方、標記「本輪新增」並以米色高亮；舊批次自動降為「較早入表」。這是代碼級規則，不由模型自行選擇順序或樣式。
- **更適合能力相對有限的模型**：模型只處理受限的語義判斷和表達，薪資、語言、資格、狀態轉換、證據綁定和高影響副作用由確定性規則兜底。
- **四條線統一為一套產品 SOP**：`tools/` 是唯一實現，`JobSearch_2026/` 只是運行實例；所有高影響操作都經過統一 gateway、狀態機、任務包、確認門和事後校驗，模型不能自行改入口或越級寫文件。
- **材料鏈完成替換**：從 lane 基礎版 CV/CL 內容母版開始，只提交當前 JD 的 bounded delta；系統編譯完整 canonical 內容，獨立上下文只審 CV/CL 內容，隨後由系統用 lane master 渲染 DOCX、轉換 PDF，再做格式門禁。Email 不進入子審計，也不由模型另起材料入口。
- **JobsDB 採用緩存優先和受控人工恢復**：正常先讀 URL JD 緩存；遇到 Cloudflare/WAF 時不無限重試。私人運行實例可在用戶自己的日常 Chrome 中彈出驗證窗口，由用戶點擊一次，系統在同一瀏覽器會話中順序復用已驗證狀態完成後續抓取；不復制 cookie、不把會話文件提交到倉庫。公開產品不包含任何個人 token 或 cookie。
- **lane 與編號邊界重新固定**：深評邊界只鎖定 URL 對應的 lane 字母；掃描預覽不分配永久編號。只有用戶確認入表時，系統才按每個 lane/層級的最新計數器順延編號，並將編號、台帳行和材料包綁定。
- **材料狀態格式固定**：fresh24 工作表首次建立時，V 列「材料狀態」由代碼寫入固定下拉選項；選擇「已投遞」會以條件格式把整行變成綠色。後續追加只沿用這套格式，不由模型決定列位、選項或顏色。
...
掃描時只分配 lane（A–F，可選 G）和匹配層級；檢索結果不會出現永久編號。
