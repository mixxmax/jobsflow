...
JobsFlow 把業務 SOP 固定成一條有邊界的流水線：`scan / push / materials / audit / format / apply / base / intent / archive / sync` 都先經統一 gateway，再調用對應功能。模型不能自行換入口或跳過狀態機；缺少確認、當前輸入或必要產物時會停下來。材料製作固定走 `materials-vnext-1`：基礎版 → 有界 JD 定製 → CV/CL 內容審計 → DOCX/PDF 格式門。
...
| `/intake` | 接收你已看過的職位 URL；標準化、去重、補齊頁面資訊，先生成只讀 proposal | 提供 URL，必要時補職位名、用人公司、lane 或完整 JD | 只包含本次新職位的入表提案；無完整 JD 標記「待審-JD不足」且不造分 | 只有你確認後才分配永久編號並寫入 tracker |
...
基礎版不是一張空模板，而是按 lane 預先核對過的完整內容母版。具體職位只提交少量
`rewrite / reorder / merge / add` 操作，系統保留沒有被提及的 block，再編譯完整材料：
...
這樣把重複勞動前置到基礎版製作階段，避免每個職位都從零重寫，也能讓能力相對有限的
模型在固定 schema 和門禁內完成高質量定製。

## 🛡️ SOP Control：換模型也不換規則

SOP Control 是 JobsFlow 的控制面，不是另一個求職引擎。它把「什麼時候可以做、需要誰確認、做完要留下什麼證據」變成項目級規則，讓不同模型或 harness 接手時仍走同一條產品流程：
...
JobsFlow 統一 gateway
        ↓  檢查規則、狀態、確認和必要輸入
業務功能（scan / push / materials / apply）
        ↓
產物、狀態和可追溯記錄
```

- **一次初始化，日常不用管理**：規則隨產品項目保存於控制面，命令入口和項目投影會自動讀取；用戶不需要每次重新向模型解釋 SOP。
...
### 受控的自動學習：記住經驗，但不自動改規則

JobsFlow 會在工作流邊界記錄短小、脫敏的糾正和執行摘要，按任務或階段聚合成待確認的
learning proposal。這條路徑不讀取或保存 JD、簡歷、CV/CL、郵件、Cookie、token 或大段
正文；普通操作不額外呼叫模型，也不會因學習模組故障而阻斷檢索、入表或材料交付。
提案只有在你明確選擇後才會進入 SOP Control、產品文檔、兩者，或僅限本次任務；模型不能
代替你確認。需要時可用 `/learn` 查看、延後或拒絕提案，讓系統逐步積累經驗而不越過你的
控制邊界。
...
這些能力是主流程契約，不是模型記憶：`temp_two_pass.sh` 只是兼容入口，實際掃描由 workflow gateway
執行。每輪運行會記錄掃描窗口、評分產物、語義狀態和刷新游標；只有產物驗證成功後才推進游標。臨時檢索
只對比最近三次臨時結果，預覽/日檢索只對比本次時間窗口，不必重讀整張台帳；即使某個門戶失敗，已成功
看到的崗位也會被記住，避免下一輪立即重複展示。
...
材料製作還有一個獨立的受控入口：主模型只提交當前 JD 的結構化定製 delta，系統合成完整
canonical CV/CL 後自動啟動獨立上下文子 Agent。子 Agent 只審 CV/CL 的 JD 映射、STAR、LLMO
位置、職位/雇主邊界、語法和殘句，不審 Email、DOCX、PDF 或版式；發現 P0/P1 時只把定位清楚
的 finding 交回主模型修訂，最多三次並對重複問題熔斷。內容通過後 Email 由宿主固定生成，
DOCX/PDF 由統一 lane-master renderer 生成，模型不能繞過這些入口。
...
掃描預覽只提供職位、lane、層級、分數、URL 和 JD 狀態，不產生永久崗位編號。
只有用戶確認 `/push` proposal 後，系統才分配崗位編號並把臺帳行寫入本地 CSV
或 Google Sheets。每個已入表的崗位編號再把臺帳行和材料包對應起來；Google
Sheets 只是可選的臺帳同步，不是 CV 或 Cover Letter 的存儲位置。
...
掃描時只分配 lane（A–G）和匹配層級；檢索結果不會出現永久編號。
用戶確認入表後，系統才會為該崗位生成壓縮了三層資訊的正式編號：
...
括號會從對外職位名中移除。文件名由主機統一生成：完整安全 stem 不超過 80 個字元時
...
材料格式和存放也不是模型可選項：只有確認 `/push` 才建立绑定的
...
JobsFlow 只有一套產品代碼、規則和狀態機。`JobSearch_2026/` 不是另一條代碼或規則線，而是直接運行這套產品的一個本地實例，用來保存你的簡歷、搜尋詞、JD、評分、臺帳和產物。這些運行數據默認被 Git 忽略；GitHub 發佈的是同一套產品代碼和空模板，不包含個人資料。SOP Control 的項目規則與私人求職運行數據分離，固定 gate 和測試已接入產品；簡歷、JD、cookie、Google 憑據或運行台賬不會發佈到 GitHub。項目身份、規則摘要、任務狀態與證據可在不同模型 / harness / worktree 間複用，切換模型不必重發明流程。
