...
方式或编号括号只作为内部元数据保存，不进入对外职位名。文件名由主机统一生成：先对
完整的候选人、用人公司和主职位做路径安全清理；只有完整文件名 stem 超过 80 个字符时，
才会压缩过长的公司法律后缀、职位级别范围或部门尾缀。未超过 80 个字符的文件名尽量
保留原始安全标签，不会每次运行都强制简称。完整公司/职位仍保留在 manifest 和材料正文中，
模型不能自行改文件名、选择第二套渲染入口或把猎头名称带入对外文件名。
