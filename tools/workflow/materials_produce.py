...
def dispatch_with_relay(
    ...
    *,
    ...
    body = dict(payload) if not relay_allowed else _strip_ticket(payload)
    ...
    if not ticket_id or not secret:
        return out
    if str(out.get("job_id") or body.get("job_id") or "") not in {"", str(body.get("job_id") or "")}:
    ...
    if out.get("run_id"):
        redeemed["run_id"] = out.get("run_id")
