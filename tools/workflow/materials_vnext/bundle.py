...
def _entity(ctx: MaterialsContext) -> JobEntity:
    ...
    return JobEntity(
        ...
        role_title_contract={
            "parser_version": role.get("parser_version"),
            "display": role.get("display") or source_role,
            "primary": role.get("primary") or source_role,
            "alternates": list(role.get("alternates") or []),
            "ambiguity_status": role.get("ambiguity_status"),
            "confirmation_needed": bool(role.get("confirmation_needed")),
            "slash_order_policy": dict(role.get("slash_order_policy") or {}),
        },
